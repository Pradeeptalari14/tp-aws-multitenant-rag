# 🔐 Multi-Tenant RAG with Metadata Filtering (AWS)

> Secure tenant isolation with AWS Bedrock and Cognito groups. Filters queries dynamically by tenant ID.
...
## 🎛️ How It Works
Open the **[Interactive Studio](https://pradeeptalari14.github.io/portfolio/tools/aws-multitenant-rag/)** in your portfolio dashboard to customize options, select webhook notification channels (Slack/Teams), toggle chat bot connections (WhatsApp Business/Telegram Bot), and download compiled deployment scripts dynamically.
...
```bash
git clone https://github.com/Pradeeptalari14/tp-aws-multitenant-rag.git
cd tp-aws-multitenant-rag
```
...
```bash
kubectl port-forward deployment/tp-aws-multitenant-rag 8080:8080 || echo "Port forwarding enabled on localhost:8080"
```
...
└── primary_config/
    └── cognito_policy.json     # Main configuration file
```
...
---
*Generated by [Multi-Tenant RAG with Metadata Filtering (AWS) Studio](https://pradeeptalari14.github.io/portfolio/tools/aws-multitenant-rag/) — [Talari Pradeep SRE Portfolio](https://pradeeptalari14.github.io/portfolio)*
